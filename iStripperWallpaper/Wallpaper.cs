...
namespace iStripperWallpaper
{
    public static class Wallpaper
    {
        ...
        public static async void ChangeWallpaper(uint monitorNumber, string url, string model, string outfit)
        {
            if (url == null)return;       
            try
            {
                ...
                if (!originalWallpaper.ContainsKey(monitorNumber))
                    originalWallpaper.Add(monitorNumber, wallpaper.GetWallpaper(monitorId.ToString()));
                ...
                if (Properties.Settings.Default.WallpaperDetails) m = AddDetails(m, wallpaper.GetMonitorRECT(monitorId), model, outfit);
                m.Save(wpfilepath);
        ...
        private static Bitmap AddDetails(Bitmap b, Rect l, string model = "Model", string outfit = "Show", int sz = 36)
        {
            var str = model + ", " + outfit;
        ...
        public static async void ChangeBrightness(string model, string outfit)
        {
            try
            {
                var wallpaper = (IDesktopWallpaper)(new DesktopWallpaperClass());  
                ...
                foreach(var kvp in initialImages)
                {
                    ...
                    if (Properties.Settings.Default.WallpaperDetails) o = AddDetails(o, wallpaper.GetMonitorRECT(monitorId), model, outfit);
